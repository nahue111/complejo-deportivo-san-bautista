...
// All IDs confirmed from live Unsplash photo pages
const u = (id, w, h) =>
      ...
      <div className={`flex gap-3 w-max ${dir === 'ltr' ? 'marquee-ltr' : 'marquee-rtl'}`}>
        {doubled.map((img, i) => (
          <div
            ...
            style={{ width: img.fw, height: img.fh }}
          >
            <img
              src={img.url}
              alt={img.label}
            ...
            <div
              ...
              style={{ background: 'linear-gradient(to top, rgba(9,9,11,0.7) 0%, transparent 55%)' }}
            />
            <span className="absolute bottom-3 left-4 text-white/60 text-[9px] font-bold uppercase tracking-widest">
              {img.label}
            </span>
          </div>
        ))}
...
export default function Gallery() {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo('.gal-header > *',
        { y: 28, opacity: 0 },
        {
          ...
          scrollTrigger: { trigger: '.gal-header', start: 'top 84%' },
        }
      )
    }, rootRef)
    return () => ctx.revert()
  ...
  return (
    <section id="instalaciones" ref={rootRef} className="py-24 md:py-32 overflow-hidden bg-[#09090B]">
      <div className="max-w-[1400px] mx-auto px-8 md:px-14 mb-12">
        <div className="gal-header flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div>
            ...
            <h2 className="mt-3 text-4xl md:text-5xl font-black tracking-tighter leading-none text-white">
              Hechas para<br />
              <span className="text-brand-red">jugar en serio.</span>
            </h2>
          </div>
          <p className="text-white/38 text-sm leading-relaxed max-w-[38ch] md:text-right">
            Canchas de primera calidad, iluminación profesional y
            un ambiente donde cada detalle importa.
          </p>
        </div>
      </div>
